...
try:
    ...
    config_file = os.path.abspath(os.path.join(HERE, '../../config.ini'))
    config = RawConfigParser()
    config.read(config_file)
    API_KEY = config.get('captcha_service', 'api_key')
    API_URL = config.get('captcha_service', 'api_url')
except Exception:
    API_KEY = ''
    API_URL = ''
...
def get_captcha_from_url(url):
    r = requests.get(API_URL.format(API_KEY, url), timeout=60)
    return r.content

if __name__ == '__main__':
    image = sys.argv[1]
    print get_captcha_from_url(image)
