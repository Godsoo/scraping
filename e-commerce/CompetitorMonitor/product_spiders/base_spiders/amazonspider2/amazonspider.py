...
class BaseAmazonSpider(BaseSpider):
    """
    Base class for spiders crawling amazon website
    """
    ...
    def _collect_lowest_price(self, new_item):
    ...
    def get_next_search_request(self, callback=None):
        ...
        if search_string is None:
            ...
            url = "file:///etc/hosts"
            request = Request(url, dont_filter=True, callback=self.dummy_callback)
            return [request]
        else:
    ...
    def get_next_search_retry_request(self, callback=None):

        search_string = ''
        search_item = {}
        retry_search_count = 0

        try:
            next_search_found = False
            while not next_search_found:
                search_string, search_item, retry_search_count = self.searches_to_retry.pop()
                if search_string and search_item and retry_search_count < self.max_retry_search_not_found_count:
                    next_search_found = True
        except IndexError:
            return []

        self.log('[AMAZON] Searching again for [%s] - %s' % (search_string, retry_search_count))

        self.current_search = search_string
        self.current_search_item = search_item
        self.retry_search_count = retry_search_count
        self.collected_items = []
        self.processed_items = False
        ...
        requests.append(Request(url, meta={'search_string': search_string,
                                           'search_item': search_item,
                                           'retry_search_count': retry_search_count},
                        dont_filter=True, callback=callback))
    ...
    def get_next_asin_request(self, callback=None):
        """
        Creates request to product details using self.asins_generator
        """
        if not self.asins_generator:
            return []
        try:
            asin, sku = next(self.asins_generator)
        except StopIteration:
            return []
        ...
        self.current_search = ''
        self.current_search_item = {'identifier': asin, 'asin': asin, 'sku': sku}
        self.collected_items = []
        self.processed_items = False
        ...
        requests.append(Request(url, meta={'search_item': self.current_search_item},
                        dont_filter=True, callback=callback))
    ...
    def get_next_asin_offers_request(self, callback=None):
        """
        Creates request to offer listing using self.asins_generator
        """
        if not self.asins_generator:
            return []
        try:
            asin, product_sku = next(self.asins_generator)
        except StopIteration:
            return []
        ...
        if isinstance(product_sku, dict):
            ...
            self.current_search_item = product_sku
        else:
            self.current_search_item = {'identifier': asin, 'asin': asin, 'sku': product_sku}
        self.collected_items = []
        self.processed_items = False
    ...
    def get_next_category_request(self, callback=None):
        """
        Creates request to product list using self.category_url_generator
        """
        if not self.category_url_generator:
            return []
        try:
            url, category_name = next(self.category_url_generator)
        except StopIteration:
            return []
        ...
        requests.append(Request(url, dont_filter=True, callback=callback, meta={'category': category_name}))
    ...
    def start_requests(self):
        ...
        if self.type == 'search':
            ...
            if not requests:
                self.errors.append("No search queries to process. Quitting")
                return []
        elif self.type == 'asins':
            self.asins_generator = self.get_asins_generator()
            requests = self.get_next_asin_request()
            if not requests:
                self.errors.append("No asins to process. Quitting")
                return []
        elif self.type == 'asins_offers':
            self.asins_generator = self.get_asins_generator()
            requests = self.get_next_asin_offers_request()
            if not requests:
                self.errors.append("No asins to process. Quitting")
                return []
            ...
            if not requests:
                self.errors.append("No categories to process. Quitting")
                return []
        else:
    ...
    def process_next_step(self, spider):
        if self.collected_items:
            ...
            if not self.processed_items and self.items_not_found_callback:
                self.items_not_found_callback(self.current_search, self.current_search_item)
                if self.retry_search_not_found and self.max_retry_search_not_found_count > 0:
                    self.searches_to_retry.append((self.current_search, self.current_search_item, self.retry_search_count + 1))
            if self.type == 'search':
    ...
    def process_collected_products(self):
        if self.type == 'category':
            self.log("[[TESTING]] Current category: %s" % self.current_category)
        self.log("[[TESTING]] Items to collects: %d" % len(self.collected_items))
        count = 0
    ...
    def process_next_search(self):
        ...
        """
        requests = self.get_next_search_request()
        if not requests and self.retry_search_not_found:
            requests = self.get_next_search_retry_request()
    ...
    def _process_product_list_item_all_sellers(self, response, item):
        ...
        """
        matched_any = False
        if self.basic_match(response.meta, self.current_search_item, item):
            if self.match(response.meta, self.current_search_item, item):
                matched_any = True
                if self.need_details_page(item):
                    new_meta = safe_copy_meta(response.meta)
                    new_meta['check_match'] = False
