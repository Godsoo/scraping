...
class DigitalrevSpider(BaseSpider):
    name = 'eservicegroup-digitalrev.com'
    ...
    def parse_products(self, response):
        ...
        for product in products:
            ...
            name = product.select('div/a/@title').extract()[0]
    ...
    def parse_product(self, response):
        ...
        l.add_xpath('identifier', '//input[@id="productId"]/@value')
        l.add_value('name', meta.get('name', ''))
        category = hxs.select('//div[@class="productCategoryBanner"]/a/text()').extract()
        if category:
            l.add_value('category', category[-1].strip())
        l.add_value('url', response.url)
        l.add_value('price', meta.get('price',0))
        l.add_xpath('image_url', '//div[@id="productShowBigImages"]/img/@src')
        yield l.load_item() 
