...
class PecheurSpider(BaseSpider):
    ...
    def parse_products(self, hxs, response):
        ...
        for product in products:
            ...
            price = product.select('.//span[contains(@class, "prix")]/text()').extract()[0]
            if product.select('.//span[contains(@class, "prix")]/span[@class="decimal"]/text()'):
                price_decimal = product.select('.//span[contains(@class, "prix")]/span[@class="decimal"]/text()').extract()[0]
            else:
                price_decimal = ',0'
            price = "".join([price, price_decimal]).replace(',', '.').strip()
