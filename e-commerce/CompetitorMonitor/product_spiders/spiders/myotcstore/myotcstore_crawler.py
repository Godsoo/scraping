...
class MyOTCStoreSpider(BaseSpider):
    ...
    def parse_product(self, response):
        ...
        products = hxs.select(u'//div[@class="CategoryProductRow"]')

        for product in products:
            product_loader = ProductLoader(item=Product(), selector=product)
            url = product.select(u'.//div[@class="CategoryProductNameLink"]/a/@href').extract()[0]
            url = urljoin_rfc(get_base_url(response), url)
            product_loader.add_value('url', url)
            product_loader.add_xpath('name', u'.//div[@class="CategoryProductNameLink"]/a/text()')
            #product_loader.add_xpath('price', u'.//span[@id="lblSalePrice"]/text()',
            #                         re=u'\$(.*)')
            product_loader.add_xpath('price', u'.//span[@class="CategoryProductPrice"]/text()',
                                     re=u'\$(.*)')
