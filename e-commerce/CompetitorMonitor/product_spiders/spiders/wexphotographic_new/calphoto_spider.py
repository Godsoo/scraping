...
class CalPhotoSpider(BaseSpider):
    name = 'wexphotographic_new-calphoto.co.uk'
    allowed_domains = ['calphoto.co.uk']
    start_urls = ['https://www.calphoto.co.uk/']

    def parse(self, response):
        # categories and subcategories
        categories = response.xpath('//div[@role="navigation"]//a[not(contains(@href, "used;"))]/@href').extract()
        for cat_href in response.xpath('//div[@role="navigation"]//a[not(contains(@href, "used;"))]/@href').extract():
        ...
        # products next page
        next = response.xpath('//li[contains(@class, "ish-pagination-list-next")]/a/@href').extract()
        if next:
    ...
    def parse_product(self, response):
        ...
        l.add_value('url', url)
        l.add_xpath('brand', '//h2[@class="productBrand-name"]/a/text()')
