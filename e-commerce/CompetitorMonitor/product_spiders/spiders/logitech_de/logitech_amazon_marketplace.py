...
class LogitechAmazonSpider(BaseAmazonSpider):
    name = 'logitech-amazon.de-marketplace'
    all_sellers = True
    ...
    def get_search_query_generator(self):
        ...
        for rownum in xrange(sh.nrows):
            ...
            self.ean_list[row[4]] = row[5].strip()
    ...
    def parse_product(self, response):
        ...
        if sku:
            ...
            if not product_info['sku']:
                ...
                if not product_info['sku']:
                    tr_xpath = 'contains(td/text(), "Herstellerreferenz") or contains(td/text(), "Herstellernummer")'
                    product_info['sku'] = ''.join(hxs.select('//tr['+tr_xpath+']/td[@class="value"]/text()').extract()).strip()
        ...
        # all sellers / lowest price
            ...
            if collect_mbc and (response.meta.get('mbc_list_url_new') or product_info['mbc_list_url_new']) and self.collect_new_products:
                # yield mbc parse
                new_meta = response.meta.copy()
                new_meta['found_item'] = product_info
                if self.type == 'search':
                ...
                mbc_list_url_new = product_info['mbc_list_url_new']
                if not mbc_list_url_new:
                    mbc_list_url_new = response.meta.get('mbc_list_url_new')
                yield Request(
                    mbc_list_url_new,
        ...
        if self.basic_match(response.meta, self.current_search_item, item):
            ...
            if self.type == 'search':
                new_meta.update({
                    ...
                    'search_item': self.current_search_item,
                    'collect_mbc': True,
                    'mbc_list_url_new': item.get('mbc_list_url_new')
                })
            yield Request(
                item['url'],
                callback=self.parse_product,
        ...
        for item in results['products']:
            if item['price'] is None:
                ...
                if self._seller_ok(item['vendor']):
                    ...
                    if 'found_item' in response.meta:
                        item['sku'] = response.meta['found_item']['sku']
                    product = self.construct_product(item, meta=response.meta, use_seller_id_in_identifier=True)
                    # self.log("[AMAZON] collect parse mbc list 1: %s" % product['identifier'])
                    if self.type == 'category':
                        yield product
                    else:
                        self._collect(product)
                ...
                if self._seller_ok(item['vendor']):
                    self.log('[AMAZON] COLLECTED ITEM => %s' % item['name'])
                    product = self.construct_product(item, meta=response.meta, use_seller_id_in_identifier=True)
                    # self.log("[AMAZON] collect parse mbc list 2: %s" % product['identifier'])
                    if self.type == 'category':
                        yield product
                    else:
                        self._collect(product)
