...
from product_spiders.base_spiders.amazonspider2.amazonspider_concurrent import BaseAmazonConcurrentSpider, AmazonUrlCreator
from product_spiders.base_spiders.amazonspider2.scraper import AmazonScraperProductDetailsException


class TheBookPeopleAmazonBuyboxSpider(BaseAmazonConcurrentSpider):
    name = 'thebookpeople-longtail-amazon.co.uk-buybox'
    ...
    def _process_product_info_product_details(self, response, product_info):
        ...
        # Buy Box
            ...
            if (product_info['price'] and product_info['vendor'] and self._seller_ok(product_info['vendor'])) or \
            ...
            elif (not product_info['vendor'] or not product_info['price']) and product_info.get('mbc_list_url_new'):
                new_meta = response.meta.copy()
                new_meta['found_item'] = product_info
                new_meta.update({
                        'search_string': response.meta['search_string'],
                ...
                self.log("[AMAZON] WARNING: product has no vendor or price: %s" % response.url)
            else:
    ...
    def parse_product(self, response):
        ...
        # Fix response.meta['search_item'] and meta['search_item']. Needed for BSM spider
        #if not response.meta.get('search_item'):
        #    response.meta['search_item'] = product_info
        #if not response.meta['search_item']:
        response.meta['search_item'] = response.meta['search_item']
