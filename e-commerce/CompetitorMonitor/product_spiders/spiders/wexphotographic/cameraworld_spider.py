...
class WexCameraWorldSpider(BaseSpider):
    name = 'wexphotographic-cameraworld.co.uk'
    allowed_domains = ['cameraworld.co.uk']
    start_urls = ['http://www.cameraworld.co.uk/used-equipment.html?infinitescroll=1&p=1']
    download_delay = 1
    brands = []
    ...
    def parse(self, response):
        ...
        for brand in brands:
            ...
            yield Request(brand_url+'&p=1', callback=self.parse_used, meta={'brand': brand_name})

        yield Request(self.start_urls[0], dont_filter=True, callback=self.parse_used)

    def parse_used(self, response):
        base_url = get_base_url(response)

        last_page = int(response.xpath('//div[@class="lastpagenumber"]/@id').extract()[0][1:])
        cur_page = int(url_query_parameter(response.url,'p'))
        if cur_page <= last_page:
            cur_page += 1
            next_page_url = add_or_replace_parameter(response.url,'p', str(cur_page))
            yield Request(next_page_url, callback=self.parse_used)

        for url in response.xpath('//div[@class="products-set"]/ul/li/h4/a/@href').extract():
    ...
    def parse_product(self, response):
        ...
        try:
            product_identifier = response.xpath('//input[@name="product"]/@value').extract()[0].strip()
        except:
            product_identifier = response.xpath('//form[@id="product_addtocart_form"]/@action').re(r'/product/(\d+)')
        ...
        product_name = response.xpath('//h2[@itemprop="name"]/text()').extract()[0]
        ...
        price = response.xpath('//*[@id="product-price-{}"]/div/span[@class="price"]/text()'.format(product_identifier)).extract()[0]
        price_pennies = response.xpath('//*[@id="product-price-{}"]/div/span[@class="price"]/span[@class="price-pennies"]/text()'.format(product_identifier)).extract()
        if price_pennies:
