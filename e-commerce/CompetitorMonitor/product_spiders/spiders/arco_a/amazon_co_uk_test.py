...
from product_spiders.base_spiders.amazonspider import BaseAmazonSpider, AmazonProductLoader
...
class ArcoAmazonCoUkSpiderTestOldBaseSpider(BaseAmazonSpider, BigSiteMethodSpider):
    name = 'arco-new-amazon.co.uk[test_old_base_spider]'
    all_sellers = False
    only_buybox = True
    ...
    def __init__(self, *args, **kwargs):
        BaseAmazonSpider.__init__(self, 'www.amazon.co.uk', *args, **kwargs)
        BigSiteMethodSpider.__init__(self, *args, **kwargs)
    ...
    def _collect_all(self, collected_items, new_item):
        ...
        if new_item['identifier']:
            if new_item['identifier'] in self.matched_identifiers:
                return
        else:
            if new_item['name'] in self.matched_identifiers:
                return
        collected = super(ArcoAmazonCoUkSpiderTestOldBaseSpider, self)._collect_all(collected_items, new_item)
        if collected:
            if new_item['identifier']:
        ...
        with open(os.path.join(HERE, 'arco_products.csv')) as f:
            ...
            for row in reader:
                if row['category'].lower() == self.category.lower():
                    yield self.search(row['description'].strip(), {
    ...
    def remove_duplicates(self, csv_file):
        ...
        with open(csv_file) as f:
            ...
            for row in list_reader:
                identifiers.append(row[FIELD].strip())
            if '' in identifiers:
                print 'WARNING! Few empty identifiers'
            # are unique?
            self.log('Diff: %s' % (len(identifiers) - len(set(identifiers)),))
            if len(identifiers) != len(set(identifiers)):
                print 'Duplicates found...'
                # removing duplicated
                groups = []
                new_file_name = '%s_new.csv' % '.'.join(csv_file.split('.')[:-1])
                self.log('INFO: Creating new file %s' % new_file_name)
                with open(new_file_name, 'w') as f_new:
                    writer = csv.DictWriter(f_new, fieldnames)
                    writer.writeheader()
                    for row in list_reader:
                        if row[FIELD] not in groups:
                            groups.append(row[FIELD])
                            writer.writerow(row)
                        else:
                            self.log('>>> Removing duplicated: %s' % row[FIELD])
                # replace the old csv file
                shutil.copy(new_file_name, csv_file)
            else:
                self.log('OK! All identifiers are unique')
