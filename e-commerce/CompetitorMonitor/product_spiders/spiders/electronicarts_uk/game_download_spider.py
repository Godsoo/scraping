...
class GameSpider(BaseSpider):
    name = 'electronicarts-uk-game.co.uk-download'
    ...
    def start_requests(self):
        with open(os.path.join(HERE, 'EAMatches.csv')) as f:
            ...
            for row in reader:
                if row['Game - Download']!="No match":
                    yield Request(row['Game - Download'], meta={'sku': row['sku']})
    ...
    def parse(self, response):
        ...
        identifier = hxs.select('//ul[contains(@class, "download")]//input[@name="productId_1"]/@value').extract()
        if not identifier:
            log.msg('No download available')
            return
        loader.add_value('identifier', identifier[0])
        name = hxs.select('//div[@class="productHeader"]/h1/text()').extract()[0].strip()
        loader.add_value('name', name)
        loader.add_value('sku', meta['sku'])
        price =  ''.join(hxs.select('//ul[contains(@class, "download")]/li[contains(@class, "price")]//text()').extract()).strip()
