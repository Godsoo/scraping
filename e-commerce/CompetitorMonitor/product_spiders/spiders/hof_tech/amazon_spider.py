...
class AmazonSpider(BaseSpider):
    name = 'houseoffraser-tech-amazon.com'
    allowed_domains = ['amazon.com']
    user_agent = 'spd'

    def start_requests(self):
        shutil.copy(os.path.join(HERE, 'houseoffraser_tech.csv'),os.path.join(HERE, 'houseoffraser_tech.csv.' + self.name + '.cur'))
        with open(os.path.join(HERE, 'houseoffraser_tech.csv.' + self.name + '.cur')) as f:
            ...
            for row in reader:
                ...
                yield Request(url % {'q': query}, meta={'price': row['price'].replace('$', ''), 'sku': sku}, dont_filter=True)
