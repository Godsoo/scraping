...
class EAAmazonSpider(BaseAmazonSpider):
    name = 'electronicarts-amazon.de-download'
    all_sellers = False
    ...
    def start_requests(self):
        with open(os.path.join(HERE, 'EAMatches.csv')) as f:
            ...
            for row in reader:
                if row['Amazon - Download']!="No matches":
                    s_item = {
                            'sku': row['sku'],
                            'brand': '',
                            'name': '',
                    ...
                    yield Request(row['Amazon - Download'], meta={'search_string': '', 'search_item': s_item}, dont_filter=True, callback=self.parse_product)
