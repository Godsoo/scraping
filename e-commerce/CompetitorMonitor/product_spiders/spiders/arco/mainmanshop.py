...
class MainmanshopSpider(BaseSpider):

    name = 'arco-mainmanshop.co.uk'
    ...
    def parse_product(self, response):
        ...
        if not self._validate_not_empty('product code', prod_code, response):
            return
        prod_codes = self._get_codes(prod_code[0].replace('Product Code:',''))
        ...
        if len(products) == 1:
            ...
            if not options:
                for prod in products.values():
                    ...
                    product['sku'] = prod['code'] + prod['option']
                ...
                for option in options:
                    ...
                    product['sku'] = prod['code'] + option_data['option']
                    product['identifier'] = product['sku']
                    product['stock'] = 1 if int(prod['stock']) else 0
                    yield self._load_item(product, response)

    def _get_codes(self, sku):
        request = urllib2.Request(self.xhr_url + sku.strip())
        request.add_header(*self.xhr_header)
        try:
            ...
            self.log('XHR [codes] :: %s' % str(e), level=log.ERROR)
