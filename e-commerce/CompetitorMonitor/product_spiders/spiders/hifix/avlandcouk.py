...
class AvlandCoUkSpider(BaseSpider):
    ...
    def parse_products(self, response):
        ...
        for product_el in products:
            ...
            sku = product_el.select('td/a/font/text()').extract()[0]
            loader.add_value('identifier', sku)
            loader.add_value('sku', sku)
            brand = product_el.select('td/font/text()').extract()[0]
            loader.add_value('brand', brand)
            image_url = product_el.select('@onmouseover').extract()
            image_url = image_url[0].split("/*url*/'")[-1].split("')")[0] if image_url else ''
            loader.add_value('image_url', image_url)
            loader.add_xpath('category', '//td/font[@size="5"]/text()')
        ...
        if not products:
            ...
            for url in hxs.select('//div/center/table[4]//a/@href').extract():
                url = urljoin_rfc(URL_BASE, url.strip())
    ...
    def parse_item(self, response):
        ...
        brand = hxs.select('//table//td/a/img/@alt').extract()[0]
        image_url = hxs.select('//table//td/a[contains(@href, "jpg")]/img/@src').extract()[0]
        identifier = response.url.split('/')[-1].replace('.asp','')
        ...
        if ('do_not_use_options' in response.meta and response.meta['do_not_use_options']) or not options:
            ...
            if not prices:
                ...
                if price:
                    ...
                    loader.add_value('category', brand)
                    loader.add_value('image_url', image_url)
                    loader.add_value('identifier', identifier+'-'+response.meta['add_name'])
                    yield loader.load_item()
                    products_found += 1
