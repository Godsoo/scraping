...
class PeterTysonSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        category = hxs.select('//div[@id="ListingHeading"]/h1/text()').extract()
    ...
    def _get_prices(self, price_response):
        ...
        image_url = hxs.select('//div[@id="gallery1"]/a/img/@src').extract()
        image_url = urljoin_rfc('http://www.petertyson.co.uk/', image_url[0]) if image_url else ''

        category = price_response.meta['category']
        ...
        if not products and prices:
            product_id = hxs.select("//form[@class='eBbuybutton']/input[@name='product']/@value").extract()
            if product_id:
            ...
            if len(subprods) == 1 and subprods[0] in prices and subprods[0] not in unpriced:
                ...
                for i, subprod in enumerate(subprods):
                    if subprod in prices and subprod not in unpriced:
                        ...
                        loader.add_value('image_url', image_url)
                        loader.add_value('category', category)
                        loader.add_value('sku', subprod)
                        loader.add_value('identifier', subprod)
                        yield loader.load_item()
        '''
