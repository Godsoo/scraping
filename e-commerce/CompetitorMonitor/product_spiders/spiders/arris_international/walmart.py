...
class WalmartSpider(BaseSpider):
    name = 'arris_international-walmart.com'
    ...
    def parse_default(self, response):
        ...
        for sku in skus:
            yield Request('http://www.walmart.com/search/?query=' + sku, callback=self.parse,
                          meta={'brand': 'SURFboard', 'sku': sku})
    ...
    def parse_product_reviews(self, response):
        ...
        for review in reviews:
            ...
            author = review.select('.//div[contains(@class, "customer-stars")]//'
                                     'span[contains(@class, "customer-name-heavy")]/text()').extract()[0]
            ...
            loader.add_value('full_text', review_text + ' by ' + author)

            date = review.select('.//span[contains(@class, "customer-review-date")]/text()').extract()
            loader.add_value('date', date[0] if date else None)

            rating_text = review.select('.//div[contains(@class, "customer-stars")]/'
                                        'span[contains(@class, "visuallyhidden")]/text()').re(r'(\d+)')
