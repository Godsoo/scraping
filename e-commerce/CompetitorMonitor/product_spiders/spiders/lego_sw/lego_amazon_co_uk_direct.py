...
class LegoAmazonSpider(BaseLegoAmazonSpider):
    name = 'legosw-amazon.co.uk-direct'
    ...
    def convert_to_pounds(self, price):
        return round(extract_price(price) / self.exchange_rate, 2)  # 1 pound = 10.67 SEK
            ...
            for row in reader:
                yield (
                    'LEGO ' + row['Product No.'],
                    {
                        ...
                        'category': row['Theme'],
                        'price': self.convert_to_pounds(row['RRP price SEK']),
                    }
                )
...
#        name = filter_category(new_item['name'], search_item['category'])
    ...
    def start_requests(self):
        yield Request('http://www.xe.com/currencyconverter/convert/?Amount=1&From=GBP&To=SEK', callback=self.start_amazon_requests)
