# -*- coding: utf-8 -*-
"""
Ticket link: https://www.assembla.com/spaces/competitormonitor/tickets/4740
...
class CamelbakDEAmazonDeMarketplace(BaseAmazonSpider):
    name = 'camelbakde_amazon.de_marketplace'
    domain = 'amazon.de'
    ...
    def parse_product_list(self, response):
        if not response.meta.get('filter_for_brand', True):
            for x in super(CamelbakDEAmazonDeMarketplace, self).parse_product_list(response):
                yield x
            return

        else:
            brand_filter_container = response.xpath('//div[@id="leftNavContainer"]//div[@id="refinements"]'
                '/h2[contains(text(), "Marke")]/following-sibling::ul[1]')
            if not brand_filter_container:
                brand_filter_container = response.xpath('//ul[@id="ref_669059031"]')
