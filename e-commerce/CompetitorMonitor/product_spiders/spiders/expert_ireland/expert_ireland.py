...
class Expert_Ireland(CrawlSpider):
    ...
    def parse_product(self, response):
        ...
        image_url = response.xpath('//img[@itemprop="image"]/@src').extract() or response.xpath('//figure[@class="image"]/img/@src').extract()
        if image_url:
    ...
    def yield_products(self, response):
        ...
        for sku in self.products:
            ...
            for product in products:
                if product['name'].lower() != first_product['name'].lower() and product.get('brand'):
