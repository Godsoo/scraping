...
class AmazonSpider(BaseSpider):
    name = 'johnlewis-trial-amazon.com'
    allowed_domains = ['amazon.com']
    user_agent = 'spd'

    def start_requests(self):
        shutil.copy(os.path.join(HERE, 'johnlewis_data.csv'),os.path.join(HERE, 'johnlewis_data.csv.' + self.name + '.cur'))
        with open(os.path.join(HERE, 'johnlewis_data.csv.' + self.name + '.cur')) as f:
            ...
            for row in reader:
                ...
                yield Request(url % {'q': query}, meta={'identifier':identifier, 'sku':sku, 'price':row['price'].replace('$', '')})
    ...
    def parse(self, response):
        ...
        for product in products:
            ...
            loader.add_value('identifier', response.meta['identifier'])
