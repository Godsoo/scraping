...
class OutlineSkinCareSpider(BaseSpider):
#    name = 'outlineskincare.co.uk'
    name = 'facethefuture-outlineskincare'
    ...
    def parse_product(self, response):
        ...
        try:
            ...
            main_name = hxs.select('//div[@class="product-name"]/h1/text()').extract()[0].strip()
        except:
        ...
        if not call2purchase:
            ...
            if not stock_:
                in_stock = False
                name = main_name + ' (Out of stock)'
        else:
            name = main_name + ' (Call for price)'
        ...
        if product_config_reg:
            ...
            for chldprd_data in iter_product(*[attr['options'] for attr in products['attributes'].values()]):

                opt_item = Product(item)
                opt_item['identifier'] = opt_item['identifier'] + ':' + ':'.join([o['id'] for o in chldprd_data])
                opt_item['name'] = main_name + ' - ' + ' - '.join([o['label'] for o in chldprd_data])
