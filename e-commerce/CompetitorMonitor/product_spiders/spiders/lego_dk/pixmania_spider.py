...
class PixmaniaSpider(Spider):
    name = 'legodk-pixmania.com'
    allowed_domains = ['pixmania.dk']
    search_url = 'https://www.pixmania.dk/api/pcm/products?categoryId=m_448&page={page}&size={size}'
    product_url = 'https://www.pixmania.dk/p/{brand_name}-{slug}-{id}?offerId={offer_id}'
    re_sku = re.compile('(\d{4,5})')

    def start_requests(self):
        url = self.search_url.format(page=0, size=10)
        yield Request(url, headers={'Language': 'da-DK'}, meta={'page': 0})
    ...
    def parse(self, response):
        ...
        for p in data['hits']['hits']:
            ...
            for offer in [p['best_offer']] + p['other_offers']:
                ...
                dealer = offer['merchant']['name'].replace(u' DKdk', u'')
