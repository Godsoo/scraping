...
class FarfetchSpider(BaseSpider):
    name = 'stylebop-farfetch.com'
    allowed_domains = ['farfetch.com']
    rows = 120
    start_urls = ['http://www.farfetch.com/changecountry?ffref=hd']

    def parse(self, response):
        yield Request('http://www.farfetch.com/changecountry/216', callback=self.parse2)
    ...
    def parse_product(response):
        ...
        identifier = hxs.select('//p[@class="mt10"]/text()').extract()
        if identifier:
            identifier = identifier[0].strip().split('ID: ')[-1]
        else:
