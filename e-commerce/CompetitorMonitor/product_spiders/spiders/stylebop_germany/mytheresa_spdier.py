...
class MyTheresaSpider(BaseSpider):
    name = 'stylebop-germany-mytheresa.com'
    allowed_domains = ['mytheresa.com']
    start_urls = ['http://www.mytheresa.com/euro_en/mzi18n/storeswitch/selectorredirect/?redirectPath=/en-de/&country=DE']

    def parse(self, response):
        base_url = get_base_url(response)
        hxs = HtmlXPathSelector(response)
 
        yield Request("http://www.mytheresa.com/en-de", callback=self.parse_site)
    ...
    def parse_site(self, response):
        ...
        for sub_category in sub_categories:
            yield Request(sub_category, dont_filter=True, callback=self.parse_category)
    ...
    def parse_category(self, response):
        ...
        if next:
            yield Request(next[0], callback=self.parse_category)
