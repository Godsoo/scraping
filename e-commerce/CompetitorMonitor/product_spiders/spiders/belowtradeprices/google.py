...
# google accounts 'btradeprices'[1..7] password: 'asdf1234..'
...
class GoogleSpider(BaseSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        with open(os.path.join(HERE, 'belowtrade.csv')) as f:
            ...
            for row in reader:
                self.identifiers[row['url']] = row['log_sku']

    def start_requests(self):
        with open(os.path.join(HERE, 'belowtrade.csv')) as f:
            reader = csv.DictReader(f)
            for i, row in enumerate(reader):
                sku = self.identifiers.get(row['url']) or row['log_sku']

                if row['search_q'] == row['sku']:
                    query = ('%s %s' % (row['brand'], row['sku'])).replace(' ', '+')
                else:
    ...
    def _get_item(self, data, i, response):
        ...
        pr['price'] += Decimal(str(data['items'][i]['product']['inventories'][0]['shipping']))
    ...
    def parse(self, response):
        ...
        mpns = filter(lambda x: 'belowtradeprices' in x['product'].get('link',['']), data['items'])
        if mpns:
            mpns = mpns[0]['product'].get('mpns', [''])[0]
            data_mpns = {'items': [item for item in data['items'] if item['product'].get('mpns',[''])[0]==mpns]}
        if not mpns or len(data_mpns['items']) <= 1:
