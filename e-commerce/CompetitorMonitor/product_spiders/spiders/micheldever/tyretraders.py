...
from micheldeverutils import find_mts_stock_code, is_product_correct, get_speed_rating, find_brand_segment, \
    get_alt_speed, unify_brand, is_run_flat
...
class TyreTradersSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        if not run_flat_found:
            run_flat_found = 'ZPS' in title.upper()
        ...
        if 'www.tyretraders.com' in name or tyre_params not in title:
            ...
            metadata['run_flat'] = 'Yes' if result or run_flat_found else 'No'
            ...
            try:
                fuel, grip, noise = hxs.select('//*[@itemprop="name"]').re('Fuel Efficiency:(\w) WetGrip:(\w) Noise dB:(\d+)')
            except:
                fuel, grip, noise = ('', '', '')
            metadata['fuel'] = fuel
            metadata['grip'] = grip
