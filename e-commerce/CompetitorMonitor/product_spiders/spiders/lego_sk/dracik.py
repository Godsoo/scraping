...
class DracikSpider(LegoMetadataBaseSpider):
    name = u'dracik.sk'
    allowed_domains = ['www.dracik.sk']
    start_urls = [
        u'http://www.dracik.sk/lego',
    ]
    errors = []
    ...
    def parse_product(self, response):
        ...
        try:
            ...
            price = hxs.select('//div[contains(@class,"akciova-cena")]//div[@class="price-box"]/text()').extract()[0].strip()
        except:
        ...
        #availability = hxs.select(u'//*[@id="page-product-detail"]//div[@class="wrap_info"]//dl/dd[contains(text(), "dostupnos\u0165 eshop")]/preceding-sibling::dt/span/text()').extract()[0].strip()
        #if availability != u'na sklade v e-shope':
        #    loader.add_value('stock', 0)
        loader.add_value('brand', 'LEGO')
        if int(price) <= 100:
            loader.add_value('shipping_cost', 4)
