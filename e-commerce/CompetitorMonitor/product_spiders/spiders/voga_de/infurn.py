...
class InfurnDESpider(BaseSpider):
    name = 'voga_de-infurn.com'
    allowed_domains = ['infurn.com']
    start_urls = ('http://www.infurn.com/de',)
    retries = {}
    ...
    def parse_product(self, response):
        ...
        if not options:
            ...
            p.add_value('price', hxs.select('//div[@id="discounted_price"]/text()').extract()[0].replace('.', '').replace(',', '.'))
