...
class RetroFurnishSpider(BaseSpider):
    name = 'voga_nl-retrofurnish.com'
    allowed_domains = ['retrofurnish.com']
    start_urls = ('http://www.retrofurnish.com/nl/',)
    ...
    def parse_product(self, response):
        ...
        if options:
            ...
            product_name = hxs.select('//h1[contains(@class,"fn")]/text()')
            if not product_name:
                product_name = hxs.select('//span[@class="h1"]/text()')
            product_name = product_name[0].extract()
            ...
            image_url = hxs.select('//img[@id="image"]/@src')
            if not image_url:
                image_url = hxs.select('//img[@id="image-main"]/@src')
        ...
        for option in options.get('childProducts').iteritems():
            ...
            product_loader.add_value('brand', brand)
            product_loader.add_value('category', response.meta.get('category') or '')
            product_loader.add_value('price', float(option[1]['finalPrice']))
            product_loader.add_value('shipping_cost', self.get_shipping_cost(float(product_loader.get_output_value('price'))))
            product = product_loader.load_item()
            if product['identifier'] in self.products_ids:
                product['name'] = self.products_ids[product['identifier']]
