...
class BedworldSpider(PrimarySpider):
    name = 'bedworld.net'
...
#        yield Request('http://www.bedworld.net/julian-bowen-cosmo-bed-frame/', callback=self.parse_product)
    ...
    def parse(self, response):
        ...
        '''
        for cat_url in hxs.select('//ul[@class="level1"]//a/@href').extract():
    ...
    def parse_list(self, response):
        ...
        for cat_url in hxs.select('//div[@class="nav-container"]/div//ul/li/a/@href').extract():
            yield Request(urljoin_rfc(base_url, cat_url), callback=self.parse_list)

        for url in hxs.select('//li[@class="item"]//a/@href').extract():
    ...
    def parse_product(self, response):
        ...
        for url in hxs.select('//a[contains(@class,"size-boxes")]/@href').extract():
            yield Request(urljoin_rfc(get_base_url(response), url), callback=self.parse_product)

        product_category = hxs.select('//div[contains(@class,"breadcrumbs")]/ul/li/a/text()').extract()[-1].strip()
        ...
        if product_config_reg:
            ...
            product_loader.add_value('category', product_category)
            product_loader.add_value('image_url', product_image)
            price = hxs.select('//span[@id="product-price-' + product_identifier + '"]//text()').re(r'([\d.,]+)')
            price = price[0] if price else 0
            product_loader.add_value('price', price)

            option_elements = []
            dropdown_elements = hxs.select('//select[contains(@class, "product-custom-options")]')
