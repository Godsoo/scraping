...
class MyDigitaLandSpider(BaseSpider):
    name = "eservices.nl-mydigitaland.com"
    allowed_domains = ["nl.mydigitaland.com"]
    start_urls = ["http://nl.mydigitaland.com/"]
    ...
    def parse_product(self, response):
        ...
        price = hxs.select('//div[contains(@class, "product-primary-column")]//p[@class="special-price"]/span[@class="price"]/text()').extract()[0]
        ...
        in_stock = hxs.select('//p[contains(@class, "availability")]/span[text()="Op voorraad"]')
        if not in_stock:
