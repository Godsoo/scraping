...
class GoWestySpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        if response.url == 'http://www.gowesty.com/' or response.url == 'http://www.gowesty.com':
            ...
            categories = zip(hxs.select('//div[@class="box_content"]/div[@id="categories"]//a/@href').extract()[11:],
                             hxs.select('//div[@class="box_content"]/div[@id="categories"]//a/text()').extract()[11:])
    ...
    def parse_product(self, response):
        ...
        for product in products:
            ...
            product_loader.add_xpath('price', './/span[@class="prod_our_price"]/strong/text()',
                                     re='.*\$(.*[0-9])')
            price = product.select('.//span[@class="prod_our_price"]/strong/text()').re(r'.*\$(.*[0-9])')
            if not price:
                price = product.select('.//span[@class="prod_sale_price"]/span/text()').re(r'.*\$(.*[0-9])')
            if not price:
            ...
            product_loader.add_value('identifier', re.search(id_regex, url[0]).groups()[0])
