...
class GorillaNutritionSpider(BaseSpider):
    name = 'demo_manufacturer-gorillanutrition.co.uk'
    ...
    def parse_product(self, response):
        ...
        sku_text = ' '.join(''.join(hxs.select('//div[@class="description"]//text()').extract()).strip().lower().split())
        sku = re.findall('product code: (.*) a', sku_text)
        if not sku:
            sku_text = ' '.join(''.join(hxs.select('//div[@class="description"]//text()').extract()).strip().lower().split())
            sku = re.findall('product code:  (.*) ', sku_text)
