"""
Original ticket: https://app.assembla.com/spaces/competitormonitor/tickets/4929-kitbag---new-site---celtic-store/details#
...
class CelticSuperStore(BaseSpider):
    name = 'kitbag-celticsuperstore.co.uk'
    allowed_domains = ['celticsuperstore.co.uk']
    start_urls = ['http://celticsuperstore.co.uk/stores/celtic/en/c/kits?cur=GBP',
                  'http://celticsuperstore.co.uk/stores/celtic/en/c/fashion?cur=GBP',
                  'http://celticsuperstore.co.uk/stores/celtic/en/c/retro-shirts?cur=GBP',
                  'http://celticsuperstore.co.uk/stores/celtic/en/c/equipment?cur=GBP']
    extracted_identifiers = []
    
    free_shipping_over = None
    shipping_cost = None
    
    cookies = {'KB_93_PriceZoneID': 3, 
            'Kitbag_T36_NoFreeDelivery': False, 
            'KB_93_Currency': 'GBP', 
            'KB_93_CurrencyId': 1,
            'KB_93_LocationID': 204,
            'KB_93_LocationISO2': 'GB',
            'KB_93_Language': 1,
            'KB_93_network': 'KITBAG'}

    def start_requests(self):
        yield Request('http://celticsuperstore.co.uk/stores/celtic/en/product/celtic-stainless-steel-bike-bottle/170140?cur=GBP',
                      callback=self.parse_shipping, cookies=self.cookies)
    ...
    def parse_shipping(self, response):
        ...
        req = FormRequest.from_response(response, formname='aspnetForm', formdata=formdata,
                                        callback=self.parse_shipping1, cookies=self.cookies)
        yield req

    def parse_shipping1(self, response):
        req = Request("http://celticsuperstore.co.uk/stores/celtic/en/basket/basket?atb=True&cur=GBP", dont_filter=True,
                      cookies=self.cookies, callback=self.parse_shipping2)
        yield req
    ...
    def parse_shipping2(self, response):
        ...
        if free_delivery_over:
            self.free_shipping_over = extract_price(free_delivery_over[0])
            self.logger.info('Free shipping over %s' % self.free_shipping_over)
    ...
    def parse(self, response):
        ...
        for url in categories:
            yield Request(response.urljoin(url), cookies=self.cookies)
