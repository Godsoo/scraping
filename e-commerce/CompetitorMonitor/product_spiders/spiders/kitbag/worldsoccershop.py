# -*- coding: utf-8 -*-
"""
Ticket link: https://www.assembla.com/spaces/competitormonitor/tickets/4932
...
class WorldSoccerShopSpider(CrawlSpider):
    name = 'kitbag_worldsoccershop.com'
    ...
    start_urls = (
        ...
        'http://www.worldsoccershop.com/shop/departments/footwear/',
        'http://www.worldsoccershop.com/departments-balls.html'
    )

    categories = LinkExtractor(allow=('shop-by-national-team',
                                      'shop-by-club',
                                      'shop/departments/footwear/',
                                      'shop/departments/balls/'))
    ...
    def __init__(self, *args, **kwargs):
        super(WorldSoccerShopSpider, self).__init__(*args, **kwargs)
        ...
        with open(teams_file) as f:
            ...
            for row in reader:
                ...
                player_name = row['HERO NAME'].strip().decode('utf')
        ...
        file_path = os.path.join(HERE, 'Exchange Rates.csv')
        sftp.get('Exchange Rates/Exchange Rates.csv', file_path)

        with open(file_path) as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row['Currency'] == 'USD-GBP':
                    self.exchange_rate = extract_price(row['Rate'])
                    break
            self.log('Exchange rate: 1 USD -> {} GBP'.format(self.exchange_rate))
    ...
    def parse_product(self, response):
        ...
        for option in options:
            ...
            product['metadata'] = {'size': SIZE_MAP.get(size.lower(), size)}
    ...
    def add_shipping_cost(self, product):
        if 'sock' in product['name'].lower():
            product['shipping_cost'] = '15.2'
        if 'short' in product['name'].lower():
            product['shipping_cost'] = '15.47'
        return product
