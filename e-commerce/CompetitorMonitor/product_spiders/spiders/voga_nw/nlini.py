...
class InteriorAddictSpider(BaseSpider):
    name = 'voga_nw-nlini.com'
    allowed_domains = ['nlini.com']
    start_urls = ('http://nlini.com/no/directory/currency/switch/currency/NOK/uenc/',)
    ...
    def parse_product(self, response):
        ...
        if not sku:
            sku = hxs.select('//div[@class="sku"]/text()').re(': (.*)')
        ...
        if product_config_reg:
            ...
            for option_identifier, option_name in products.iteritems():
                ...
                for category in response.meta.get('categories', []):
            ...
            price = extract_price_eu(hxs.select('//*[@id="product-price-%s"]/text()' % identifier ).extract()[0])
            product_loader = ProductLoader(item=Product(), response=response)
            product_loader.add_value('name', product_name)
            product_loader.add_value('url', response.url)
            for category in response.meta.get('categories', []):
