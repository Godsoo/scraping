...
class ThesleepshopCoUkSpider(PrimarySpider):
    name = 'thesleepshop.co.uk'
    allowed_domains = ['thesleepshop.co.uk']
    csv_file = 'thesleepshop.co.uk_products.csv'

    def __init__(self, *args, **kwargs):
        super(ThesleepshopCoUkSpider, self).__init__(*args, **kwargs)
    ...
    def parse_product(response):
        ...
        product_loader.add_xpath('price', u'//div[@align="left"]/div/div[contains(text(),"Now")]/text()')
        if not product_loader.get_output_value('price'):
            product_loader.add_xpath('price', u'//div[@id="price_inside"]//span//text()')
        if not product_loader.get_output_value('price'):
            product_loader.add_xpath('price', u'//div[@id="price_inside"]//span/@ppraw')
        if not product_loader.get_output_value('price'):
        ...
        for opt_price, opt_name in multiply(opt_groups):
            ...
            try:
                prod['price'] = (Decimal(prod['price']) + Decimal(opt_price)).quantize(Decimal('1.00'))
            except TypeError:
