...
class ChainReactionCyclesComSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        action_url = response.urljoin(change_loc_form.select('./@action').extract()[0])
        input_vals = dict(zip(change_loc_form.select('.//input/@name').extract(),
                              change_loc_form.select('.//input/@value').extract()))
        params = {u'/atg/userprofiling/ProfileFormHandler.value.country': u'AU',
                  u'/atg/userprofiling/ProfileFormHandler.value.currency': u'AUD',
