...
class TyresOnTheDriveSpider(BaseSpider):
    name = 'tyresonthedrive.com_test'
    allowed_domains = ['tyresonthedrive.com']
    start_urls = ('http://www.tyresonthedrive.com/search/',)
    tyre_sizes = []
    new_old_names = {}
    brands = []
    manually_matched = []
    ...
    def start_requests(self):
        url = "http://www.tyresonthedrive.com/tyresearchstep1.aspx/PrepStep2FromTyreSearch"
        for row in self.tyre_sizes:
            tyre_size = row['Width'] + '_' + row['Aspect Ratio'] + '_' + row['Rim'] + '_' + row['Load rating']
            params = {'enteredPostcode': 'CV47 0RB', 'selectedTyreFromSize': tyre_size}
            req = Request(url,
                          method='POST',
                          dont_filter=True,
                          body=str(params),
    ...
    def parse2(self, response):
        ...
        url = "http://www.tyresonthedrive.com/tyresearchstep2.aspx/Search"
        req = Request(url,
                      method='POST',
                      dont_filter=True,
    ...
    def parse_products(self, response):
        ...
        for product_el in products:
            ...
            image_url = 'http://www.tyresonthedrive.com/img/treads/' + product_el[u'ProductTreadPattern'][u'TreadPatternImage'] + '.jpg'
            ...
            metadata['run_flat'] = 'Yes' if product_el[u'ProductAttributes'][u'IsRunFlat'] else 'No'
