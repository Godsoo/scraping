...
class ScheideggerNlSpider(BaseSpider):
    ...
    def __init__(self, *args, **kwargs):
        super(ScheideggerNlSpider, self).__init__(*args, **kwargs)
        self.names = {}
        with open(os.path.join(here, 'products.csv')) as f:
            reader = csv.DictReader(f)
            for row in reader:
                self.names[row['url']] = row['name']
    ...
    def parse_course(self, response):
        ...
        if response.url in self.names:
            product_loader.add_value('name', self.names[response.url])
        else:
