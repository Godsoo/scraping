...
class siapets_spider(BaseSpider):
        ...
        if re.search('product_group\.asp\?dept_id\=', str(response.url)):
            ...
            for p in products:
                ...
                if name:
                    ...
                    idents = re.search(r'dept_id=(?P<dept>\d+)&pf_id=(?P<pf>\d+)', res['url'])
                    res['identifier'] = '%s_%s' % (idents.group('dept'), idents.group('pf'))
    ...
    def parse(self, response):
        ...
        if re.search('product\.asp\?dept_id\=', str(response.url)):
            ...
            price = hxs.select('//table/tr/td/span[@class="pricepp"]/text()').re(r'\xa3([0-9\. \,]+)')
            if price:
                price = price[0]
            else:
                price = hxs.select('//table/tr/td/span[@class="price_largepp"]/text()').re(r'\xa3([0-9\. \,]+)')
                if price:
                    price = price[0]
            url = response.url
