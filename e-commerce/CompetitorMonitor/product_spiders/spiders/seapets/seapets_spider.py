            ...
            for p in products:
                if p.select(".//img[@class='add-to-basket']").extract():
                    ...
                    if name:
                      ...
                      identifier = p.select('.//a[contains(@onclick, "iposAddToBasket")]/@onclick').re(r'iposAddToBasket\(\'(\d+)\',')[0]
    ...
    def parse(self, response):
        ...
        for p in products:
            ...
            if p.select("./td[@style='font-size: 12px;']").extract():
                ...
                identifier = p.select('.//input[contains(@name, "stkcode-")]/@value')[0].extract().strip()
