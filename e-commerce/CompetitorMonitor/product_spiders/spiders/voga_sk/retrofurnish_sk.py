...
class RetroFurnishSkSpider(BaseSpider):
    ...
    start_urls      = ('http://www.retrofurnish.com/sk',)
    ...
    def parse(self, response):
        ...
        yield Request(
            url="http://www.retrofurnish.com/sk",
            cookies={"currency:": "SEK"}, 
    ...
    def parse_real(self, response):
        ...
        for category in categories:
            ...
            category = category.select('./span/text()').extract()
            yield Request(urljoin_rfc(base_url, url), callback=self.parse_pagination, meta={'category': category})
    ...
    def parse_product(self, response):
        ...
        url = 'http://www.retrofurnish.com/sk/' + response.url.split('/')[-1]
        ...
        for option in options.get('childProducts').iteritems():
            ...
            price = float(option[1]['finalPrice'])
            shipping = self.get_shipping_cost(price)
