...
class HalfordsComSpider(Spider):
    ...
    def parse(self, response):
        for url in response.css('aside.pillar-mainSideNav a::attr(href)').extract():
            ...
            if not price:
                ...
                if price:
                    ...
                    if product['identifier'] not in self.identifiers:
                        self.identifiers.add(product['identifier'])
                        yield product
        ...
        try:
            ...
            if item:
                ...
                if price:
                    ...
                    product = product_loader.load_item()
                    metadata = CRCMeta()
                    metadata['rrp'] = response.meta.get('rrp')
                    product['metadata'] = metadata
    ...
    def closing_parse_simple(self, response):
        for product in super(HalfordsComSpider, self).closing_parse_simple(response):
            if isinstance(product, Product) and product['identifier'] not in self.identifiers:
                self.identifiers.add(product['identifier'])
                yield product
