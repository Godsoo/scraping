...
class HpdonlineSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        with open('address_temp.csv', 'r') as f:
            ...
            for index, address in enumerate(content):
                if index == 0:
                    continue
                self.address_list.append({'Borough': address[0], 'Number': address[1], 'Street': address[2]})
    ...
    def parse_first(self, response):
        ...
        item['Borough'] = address['Borough']
        item['Number'] = address['Number']
        item['Street'] = address['Street']

        item['address'] = response.xpath('//table[@id="mymaintable_BldgInfo"]/tr/td/span[@id="mymaintable_lblAddr"]/text()').extract_first()
